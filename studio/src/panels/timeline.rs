use egui::{Ui, RichText, Color32, Vec2, Stroke, Align, Layout};
use crate::app::{EditorApp, TEXT_DIM, TEXT_PRIMARY, RED, BORDER};

pub fn ui(app: &mut EditorApp, ui: &mut Ui) {
    // Transport Controls — compact horizontal bar
    ui.horizontal(|ui| {
        ui.label(RichText::new("TIMELINE").color(TEXT_DIM).strong().size(10.0));
        ui.add_space(8.0);
        ...
        if ui.small_button(RichText::new(play_icon).size(11.0).color(play_color)).clicked() {
        ...
        .show(ui, |ui| {
            ...
            while tm <= dur {
                ...
                painter.line_segment(
                    [egui::pos2(x, origin.y + ruler_h - 5.0), egui::pos2(x, origin.y + ruler_h)],
            ...
            for (i, e) in app.world.entities.iter().enumerate() {
                ...
                if let Some(tl) = &e.components.timeline {
                    ...
                    if is_sel {
                        painter.rect_stroke(r, 3.0, Stroke::new(1.5, Color32::WHITE), egui::StrokeKind::Inside);
                    }
