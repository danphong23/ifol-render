...
pub enum Interpolation {
    ...
    Linear,
    CubicBezier {
        #[serde(default = "default_cb_x1")]
        x1: f32,
        #[serde(default)]
        y1: f32,
        #[serde(default = "default_cb_x2")]
        x2: f32,
        #[serde(default = "default_cb_y2")]
        y2: f32,
    },
    Bezier {
        #[serde(default = "default_tangent")]
        out_x: f32,
        #[serde(default)]
        out_y: f32,
        #[serde(default = "default_tangent")]
...
fn default_cb_x1() -> f32 {
    0.42
}
fn default_cb_x2() -> f32 {
    0.58
}
fn default_cb_y2() -> f32 {
    1.0
}
...
fn cubic_bezier_eval(t: f32, x1: f32, y1: f32, x2: f32, y2: f32) -> f32 {
    let mut u = t;
    for _ in 0..8 {
        let bx = 3.0 * (1.0 - u).powi(2) * u * x1 + 3.0 * (1.0 - u) * u.powi(2) * x2 + u.powi(3);
    ...
    3.0 * (1.0 - u).powi(2) * u * y1 + 3.0 * (1.0 - u) * u.powi(2) * y2 + u.powi(3)
}
...
// ══════════════════════════════════════
// Vec4 Track (for color uniforms)
// ══════════════════════════════════════

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Vec4Keyframe {
    pub time: f64,
    pub value: [f32; 4],
    #[serde(default)]
    pub interpolation: Interpolation,
...
pub struct Vec4Track {
    #[serde(default)]
    pub keyframes: Vec<Vec4Keyframe>,
}

impl Vec4Track {
    /// Evaluate the track at a given time, returning interpolated [f32; 4].
    pub fn evaluate(&self, time: f64, default_val: [f32; 4]) -> [f32; 4] {
        if self.keyframes.is_empty() {
            return default_val;
        }
        if time <= self.keyframes[0].time {
        ...
        for window in self.keyframes.windows(2) {
            ...
            if time >= a.time && time < b.time {
                ...
                return [
                    ...
                    a.value[2] + (b.value[2] - a.value[2]) * eased_t,
                    a.value[3] + (b.value[3] - a.value[3]) * eased_t,
                ];
            }
        }
        default_val
