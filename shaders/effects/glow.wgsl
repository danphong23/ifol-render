...
fn fs_main(in: VertexOutput) -> @location(0) vec4f {
    ...
    // ── Composite: Original OVER Glow (in straight-alpha space) ──
    // Standard source-over with straight alpha values:
    let out_a = orig_a + glow_alpha * (1.0 - orig_a);
    
    var out_rgb = glow_color; // default to glow color if no original
    if (out_a > 0.001) {
        // Weighted blend of original and glow colors by their contribution
        out_rgb = (orig_rgb * orig_a + glow_color * glow_alpha * (1.0 - orig_a)) / out_a;
    }
    
    // Output STRAIGHT alpha — hardware SrcAlpha blend will premultiply correctly
